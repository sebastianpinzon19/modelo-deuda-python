...
try {
    ...
    // Ejecutar el debug Python
    $pythonScript = 'PROVCA/debug_balance_python.py';
    ...
    if ($returnCode !== 0) {
        ...
        throw new Exception("Error ejecutando Python debug: $errorMessage");
    }

    // Leer resultados del archivo JSON generado por Python
    $resultsFile = 'debug_balance.json';
    if (!file_exists($resultsFile)) {
        throw new Exception('No se generó el archivo de debug');
    }
    ...
    if ($results === null) {
        throw new Exception('Error al parsear los resultados JSON de debug');
    }

    // Eliminar archivo de debug temporal
    unlink($resultsFile);
