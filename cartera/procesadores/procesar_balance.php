...
try {
    ...
    $nombreUnico = generarNombreUnico('balance', $extension);
    ...
    escribirLog("Archivo balance subido: $nombreOriginal -> $rutaDestino");

    // Ejecutar script de Python para procesar balance
    try {
        $output = ejecutarScriptPython(SCRIPT_BALANCE, $rutaDestino);
        escribirLog("Procesamiento de balance completado para: $nombreOriginal");
    } catch (Exception $e) {
    ...
    $archivosResultado = glob(DIR_RESULTADOS . '*balance*' . $timestamp . '*');
    ...
    $respuesta = [
        'success' => true,
        'message' => 'Archivo de balance procesado exitosamente',
        'data' => [
            ...
            'timestamp' => $timestamp,
            'tipo_procesamiento' => 'Balance Completo',
    ...
    escribirErrorLog("Error en procesamiento de balance: " . $e->getMessage(), $e);
