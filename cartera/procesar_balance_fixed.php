...
try {
    ...
    if (!file_exists($resultsFile)) {
        throw new Exception('Archivo de resultados no encontrado. Salida de Python: ' . implode("\n", $output));
    }
    ...
    if (empty(trim($resultsJson))) {
        throw new Exception('Archivo de resultados está vacío');
    }

    $results = json_decode($resultsJson, true);
    if ($results === null) {
        $jsonError = json_last_error_msg();
        throw new Exception('Error al parsear JSON: ' . $jsonError . '. Contenido: ' . substr($resultsJson, 0, 500));
    }
